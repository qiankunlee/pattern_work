...
class GaussianClassifiers(Dataset, Methodset):
    ...
    def rda_classification(self,test_data, gamma = 0.3, beta = 0.00, training_data = None):
...
if __name__ == "__main__":
    ...
    data_set = Dataset()
    data_set.importdata(url=data_url, index_col = 0, index_name=attribute_information, usecol=usecol)
    data_set.name = "Breast Cancer Wisconsin (Diagnostic) Data Set"
    url_classification_work(data_set)
    print "实验5-------------------------------Haberman_Dataset--------------------------------------------------------------------"
    ...
    data_set.name = "Haberman_Dataset"
    url_classification_work(data_set)
